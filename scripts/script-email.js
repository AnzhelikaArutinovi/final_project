...
emailSubscription.addEventListener("keyup", function () {
  let emailSubscriptionValue = document.getElementById("email-subscription").value;
  let regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
  ...
  if (emailSubscriptionValue.match(regex)) {
    ...
    document.getElementById("error-email-subscribe").classList.add("green-valid");
  } else {
    emailError.innerText = "Your email is invalid";
    document.getElementById("error-email-subscribe").classList.remove("green-valid");
  }
